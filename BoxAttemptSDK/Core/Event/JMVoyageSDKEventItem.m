...
@interface JMVoyageSDKEventItem ()
...
@implementation JMVoyageSDKEventItem
...
+ (instancetype)event:(NSInteger)eventId
            ...
            errorCode:(NSInteger)errorCode
         errorMessage:(NSString *)errorMessage {
    JMVoyageSDKEventItem *event = [[JMVoyageSDKEventItem alloc] init];
    event.eventId = eventId;
    if (exStrs.count > 0) event.exStr1 = exStrs[0];
    if (exStrs.count > 1) event.exStr2 = exStrs[1];
    event.openId = [JMVoyageSDKInfo shared].openId?:@"";
    event.recordTime = [JMValue TimestampMillisecond];
    event.netType = [JMVoyageSDKRequest shared].networkReachabilityStatusString;
    
    event.errorCode = errorCode;
    event.errorMessage = errorMessage;
    
    JMLog(@"JMVoyage Event Created = %zd(%@): %@", eventId, exStrs, errorMessage);
