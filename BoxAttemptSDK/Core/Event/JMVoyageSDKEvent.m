...
@implementation JMVoyageSDKEvent
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.directoryPath = [documentsDirectory stringByAppendingString:@"/JMVoyageSDK/Event/"];
...
- (void)uploadEvents:(NSArray <JMVoyageSDKEventItem *>*)events {
    ...
    [[JMVoyageSDKRequest shared] uploadEvents:events
...
- (void)recordEvents:(NSArray <JMVoyageSDKEventItem *>*)events {
...
- (BOOL)save {
    ...
    for (JMVoyageSDKEventItem *event in self.eventList) {
...
- (NSArray <JMVoyageSDKEventItem *> *)read:(NSString *)fileName {
    ...
    for (NSDictionary *eventDict in eventDictList) {
        [eventList addObject:[JMVoyageSDKEventItem eventWithDict:eventDict]];
    }
...
- (void)uploadHistory {
    ...
    for (NSInteger i = 0; i < fileList.count && i <= 1; i++) {
        ...
        NSArray <JMVoyageSDKEventItem *>* events = [self read:fileName];
